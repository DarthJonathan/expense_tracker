<script lang="ts">
	...
	import { AlertTriangle, ArrowLeft, CheckCircle2, ChevronRight, FileText, Link2, RefreshCw, Search, ShieldCheck, Trash2, Unlink, Upload } from 'lucide-svelte';
	import { deleteLocalStatementJob, getLocalStatementJobs, putLocalStatementJob } from '$lib/db';
	import { combinedStatementRowsCompatibilityError, compatibleCombinedMatchEntries, countStatementReviews, filterStatementRows, matchedMasterUpdateCount, newMasterTransactionCount } from '$lib/statement-review';
	import { createCombinedStatementNew } from '$lib/statements';
	import type { CombinedStatementTransaction } from '$lib/types';
	import { confirmStatementIngestion, createCombinedStatementMatch, createStatementIngestion, deleteStatementIngestion, deleteStatementRow, dissolveCombinedStatementMatch, extractEmbeddedPdfText, getStatementIngestion, listStatementIngestions, parseDbsStatementText, updateStatementRow } from '$lib/statements';
	import StatementMatchReview from './StatementMatchReview.svelte';
	...
	let combineSearch = '';
	let combinedMatchExpenseId = '';
	let combineMode: 'new' | 'matched' = 'new';
	let combinedDraft: CombinedStatementTransaction = { merchant: '', occurredOn: '', categoryId: '', note: '' };
	let combining = false;
	let dissolvingCombinedMatchId = '';

	$: if (!selectedAccountId && accounts.length) selectedAccountId = accounts[0].id;
	$: filteredRows = filterStatementRows(detail?.rows ?? [], searchQuery, statusFilter);
	$: reviewCounts = countStatementReviews(detail?.rows ?? []);
	$: selectedRows = (detail?.rows ?? []).filter((row) => selectedRowIds.has(row.id));
	$: matchedUpdates = matchedMasterUpdateCount(detail?.rows ?? []);
	$: newTransactions = newMasterTransactionCount(detail?.rows ?? []);
	$: combineTotal = selectedRows.reduce((total, row) => total + row.amount, 0);
	$: combineError = combinedStatementRowsCompatibilityError(selectedRows);
	$: combineTarget = entries.find((entry) => entry.id === combinedMatchExpenseId);
	$: combinedGroups = [...new Set((detail?.rows ?? []).map((row) => row.combinedMatchId).filter(Boolean))].map((id) => ({ id: id!, rows: (detail?.rows ?? []).filter((row) => row.combinedMatchId === id) }));
	$: compatibleCombineEntries = compatibleCombinedMatchEntries(selectedRows, entries, accounts, categories, combineSearch);
	...
	async function openIngestion(ingestion: StatementIngestion): Promise<void> {
		...
		try { detail = await getStatementIngestion(groupId, ingestion.id); detailCache = { ...detailCache, [ingestion.id]: detail }; searchQuery = ''; statusFilter = 'all'; clearCombineSelection(); }
		catch (error) { actionError = messageFor(error); }
		finally { detailLoading = false; }
	}
	function closeDetail(): void { detail = null; editingRowId = ''; rowDraft = null; searchQuery = ''; statusFilter = 'all'; clearCombineSelection(); }
	...
	async function removeRow(): Promise<void> {
		if (!detail || !rowDraft || removingRowId) return;
		const draft = rowDraft;
		if (!window.confirm(`Remove ${draft.merchant || 'this transaction'} from staging? It will not be created or matched during confirmation.`)) return;
		actionError = ''; removingRowId = draft.id;
		try {
			const updated = await deleteStatementRow(groupId, detail.ingestion.id, draft.id);
			detail = updated; detailCache = { ...detailCache, [updated.ingestion.id]: updated }; upsertIngestion(updated.ingestion); cancelEdit(); actionMessage = `Removed ${draft.merchant || draft.sourceRowKey} from staging.`;
		} catch (error) { actionError = messageFor(error); }
		finally { removingRowId = ''; }
	}
	function toggleCombinedSelection(row: StatementIngestionRow): void {
		if (combining || row.combinedMatchId || detail?.ingestion.status === 'confirmed') return;
		if (!selectedRowIds.size) combinedDraft = { merchant: row.merchant, occurredOn: row.occurredOn ?? '', categoryId: row.categoryId ?? '', note: '' };
		cancelEdit();
		const next = new Set(selectedRowIds);
		if (next.has(row.id)) next.delete(row.id); else next.add(row.id);
		selectedRowIds = next;
		combinedMatchExpenseId = '';
	}
	function clearCombineSelection(): void { selectedRowIds = new Set(); combineSearch = ''; combinedMatchExpenseId = ''; combinedDraft = { merchant: '', occurredOn: '', categoryId: '', note: '' }; }
	async function combineAndMatch(): Promise<void> {
		if (!detail || combining) return;
		const compatibilityError = combinedStatementRowsCompatibilityError(selectedRows);
		if (compatibilityError) { actionError = compatibilityError; return; }
		const creating = combineMode === 'new';
		if (!creating && !combinedMatchExpenseId) { actionError = 'Choose the existing transaction to update.'; return; }
		if (creating && (!combinedDraft.merchant.trim() || !combinedDraft.occurredOn || !combinedDraft.categoryId)) { actionError = 'Enter a merchant, date, and category for the new transaction.'; return; }
		actionError = ''; combining = true;
		try {
			const updated = creating
				? await createCombinedStatementNew(groupId, detail.ingestion.id, selectedRows.map((row) => row.id), combinedDraft)
				: await createCombinedStatementMatch(groupId, detail.ingestion.id, selectedRows.map((row) => row.id), combinedMatchExpenseId);
			detail = updated; detailCache = { ...detailCache, [updated.ingestion.id]: updated }; upsertIngestion(updated.ingestion); clearCombineSelection(); actionMessage = creating ? 'Combined transaction saved for review. One new transaction will be created when you confirm the import.' : 'Rows combined and matched. The master transaction will be updated once when you confirm the import.';
		} catch (error) { actionError = messageFor(error); }
		finally { combining = false; }
	}
	async function uncombine(combinedMatchId: string): Promise<void> {
		if (!detail || dissolvingCombinedMatchId || !window.confirm('Uncombine these statement rows? Each row will return to review.')) return;
		actionError = ''; dissolvingCombinedMatchId = combinedMatchId;
		try {
			const updated = await dissolveCombinedStatementMatch(groupId, detail.ingestion.id, combinedMatchId);
			detail = updated; detailCache = { ...detailCache, [updated.ingestion.id]: updated }; upsertIngestion(updated.ingestion); clearCombineSelection(); actionMessage = 'Combined match removed; its rows are ready for individual review.';
		} catch (error) { actionError = messageFor(error); }
		finally { dissolvingCombinedMatchId = ''; }
	}
	...
	function cachedSummary(ingestion: StatementIngestion): string {
		...
		const counts = countStatementReviews(cached.rows); const updates = matchedMasterUpdateCount(cached.rows); const creates = newMasterTransactionCount(cached.rows); return ingestion.status === 'confirmed' ? `${creates} created · ${updates} updated` : `${updates} matched · ${creates} new${counts.unreviewed ? ` · ${counts.unreviewed} to review` : ''}`;
	}
...
<section class="statement-ingestion" aria-label="Statement ingestion">
	...
	{:else}
		...
		{#if actionError}<p class="feedback error" role="alert"><AlertTriangle size={17} />{actionError}</p>{/if}
		{#if actionMessage}<p class="feedback success"><CheckCircle2 size={17} />{actionMessage}</p>{/if}
		<div class="summary-grid"><article><span>Review progress</span><strong>{detail.rows.length - reviewCounts.unreviewed} / {detail.rows.length}</strong><small>{reviewCounts.unreviewed ? `${reviewCounts.unreviewed} still need review` : 'All rows reviewed'}</small></article><article><span>Matched</span><strong>{matchedUpdates}</strong><small>Master transaction{matchedUpdates === 1 ? '' : 's'} to update</small></article><article><span>New</span><strong>{newTransactions}</strong><small>Transactions to create</small></article><article><span>Statement total</span><strong>{money(detail.ingestion.statementGrandTotal ?? detail.ingestion.declaredNewTransactionsTotal ?? 0)}</strong><small>{detail.ingestion.paymentDueDate ? `Due ${shortDate(detail.ingestion.paymentDueDate)}` : 'Reconciliation amount'}</small></article></div>
		...
		<section class="transactions-card">
			...
			{#if selectedRows.length}
				<form class="combine-panel" on:submit|preventDefault={() => void combineAndMatch()}>
					<div class="combine-summary"><Link2 size={18} /><div><strong>Combine {selectedRows.length} statement rows</strong><small>{accountFor(selectedRows[0].accountId ?? '')?.name} · {selectedRows[0].type === 'income' ? 'Income / credit' : 'Expense'}</small></div><strong class="combine-total">{combineError && selectedRows.length > 1 ? 'Check selection' : money(combineTotal, selectedRows[0].currency)}</strong></div>
					<div class="combine-mode" role="group" aria-label="Combined transaction action"><button type="button" aria-pressed={combineMode === 'new'} on:click={() => combineMode = 'new'} disabled={combining}>Create new transaction</button><button type="button" aria-pressed={combineMode === 'matched'} on:click={() => combineMode = 'matched'} disabled={combining}>Match existing</button></div>
					{#if combineMode === 'new'}
						<div class="combine-fields">
							<label class="wide-field">Merchant<input bind:value={combinedDraft.merchant} required disabled={combining} /></label>
							<label>Date<input type="date" bind:value={combinedDraft.occurredOn} required disabled={combining} /></label>
							<label>Category<select bind:value={combinedDraft.categoryId} required disabled={combining}><option value="">Choose category</option>{#each visibleCategories(selectedRows[0]) as category}<option value={category.id}>{category.name}</option>{/each}</select></label>
							<label class="wide-field">Note · optional<input bind:value={combinedDraft.note} placeholder="Add a note for the combined transaction" disabled={combining} /></label>
						</div>
						<p class="combine-hint">One transaction will be created on final import confirmation. Original statement rows stay linked.</p>
					{:else}
						<label class="combine-search"><Search size={16} /><input bind:value={combineSearch} type="search" placeholder="Search merchant, date or amount" aria-label="Find existing transaction to combine" disabled={combining} /></label>
						<div class="combine-candidates">{#each compatibleCombineEntries as entry}<label class="combine-candidate"><input type="radio" name="combined-master" bind:group={combinedMatchExpenseId} value={entry.id} disabled={combining} /><span><strong>{entry.merchant}</strong><small>{shortDate(entry.occurredOn)} · {categories.find(category => category.id === entry.categoryId)?.name ?? 'Uncategorised'}</small></span><strong>{money(entry.amount, entry.currency)}</strong></label>{:else}<p class="combine-hint">No matching transactions. Try another search or create a new transaction.</p>{/each}</div>
						{#if combineTarget}<p class="combine-hint">Amount after import <strong>{money(combineTarget.amount, combineTarget.currency)} → {money(combineTotal, selectedRows[0].currency)}</strong></p>{/if}
						<p class="combine-hint">Keep existing merchant, date, category and note.</p>
					{/if}
					{#if combineError}<p class="combine-hint" role="status">{combineError}</p>{/if}
					<div class="combine-actions"><button class="combine-cancel" type="button" on:click={clearCombineSelection} disabled={combining}>Cancel</button><button class="inline-primary" type="submit" disabled={combining || Boolean(combineError) || (combineMode === 'matched' && !combinedMatchExpenseId) || detail.ingestion.status === 'confirmed'}><Link2 size={17} />{combining ? 'Saving…' : combineMode === 'new' ? 'Combine as new' : 'Combine & match'}</button></div>
				</form>
			{/if}
			{#each combinedGroups as group (group.id)}
				{@const draft = group.rows[0].combinedTransaction}
				{@const target = entries.find(entry => entry.id === group.rows[0].matchExpenseId)}
				<div class="saved-combination"><Link2 size={18} /><div><strong>{group.rows.length} rows · {draft ? 'One new transaction' : 'One matched transaction'}</strong><small>{draft?.merchant ?? target?.merchant ?? 'Existing transaction'} · {money(group.rows.reduce((total, row) => total + row.amount, 0), group.rows[0].currency)}</small>{#if draft}<small>{shortDate(draft.occurredOn)} · {categories.find(category => category.id === draft.categoryId)?.name ?? 'Category unavailable'}{draft.note ? ` · ${draft.note}` : ''}</small>{/if}</div>{#if detail.ingestion.status !== 'confirmed'}<button class="combine-cancel" type="button" on:click={() => void uncombine(group.id)} disabled={Boolean(dissolvingCombinedMatchId) || combining}><Unlink size={15} />{dissolvingCombinedMatchId === group.id ? 'Uncombining…' : 'Uncombine'}</button>{:else}<span class="combined-badge">Imported</span>{/if}</div>
			{/each}
			...
			<div class="transaction-list">
				{#each filteredRows as row (row.id)}
					<article class:row-warning={row.warningCodes.length > 0} class:editing={editingRowId === row.id} class:selected-for-combine={selectedRowIds.has(row.id)} class="transaction-row">
						<label class="row-selector" title={row.combinedMatchId ? 'Uncombine this group before selecting rows' : 'Select for combined match'}><input type="checkbox" checked={selectedRowIds.has(row.id)} on:change={() => toggleCombinedSelection(row)} disabled={Boolean(row.combinedMatchId) || detail.ingestion.status === 'confirmed'} /><span class="selection-check" aria-hidden="true"></span><span class="sr-only">Select {row.merchant || row.sourceRowKey} for a combined match</span></label>
						<button class="row-summary" type="button" on:click={() => editingRowId === row.id ? cancelEdit() : editRow(row)} disabled={detail.ingestion.status === 'confirmed' || Boolean(row.combinedMatchId)}><span class="date-cell" data-label="Date">{shortDate(row.occurredOn)}</span><span class="merchant-cell" data-label="Merchant"><strong>{row.merchant || 'Unreadable transaction'}</strong><small>{row.cardholder}{row.statementReference ? ` · ${row.statementReference}` : ''}</small></span><span class="amount-cell" data-label="Amount"><strong class:credit={row.type === 'income'}>{row.type === 'income' ? '−' : ''}{money(row.amount, row.currency)}</strong>{#if row.foreignAmount != null && row.foreignCurrency}<small>{money(row.foreignAmount, row.foreignCurrency)} original</small>{/if}</span><span class="category-cell" data-label="Category">{categories.find((category) => category.id === row.categoryId)?.name ?? 'Uncategorised'}</span><span class={`row-status ${row.reviewStatus}`}>{reviewLabel(row.reviewStatus)}{#if row.combinedMatchId}<span class="combined-badge">Combined</span>{/if}</span><span class="row-chevron"><ChevronRight size={18} /></span></button>
						{#if row.combinedMatchId}<button class="uncombine-row" type="button" on:click={() => void uncombine(row.combinedMatchId ?? '')} disabled={dissolvingCombinedMatchId === row.combinedMatchId || detail.ingestion.status === 'confirmed'}><Unlink size={15} />{dissolvingCombinedMatchId === row.combinedMatchId ? 'Uncombining…' : 'Uncombine'}</button>{/if}
						{#if editingRowId === row.id && rowDraft}
							<div class="row-editor">
								<div class="editor-grid">
									<label>Date<input aria-label="Transaction date" type="date" bind:value={rowDraft.occurredOn} /></label>
									<label class="merchant-field match-field">Merchant<input aria-label="Merchant" bind:value={rowDraft.merchant} /></label>
									<label>Amount<input aria-label="Amount" type="number" min="0" step="0.01" value={rowDraft.amount / 100} on:change={changeAmount} /></label>
									<label>Kind<select aria-label="Statement row kind" bind:value={rowDraft.statementKind}><option value="transaction">Transaction</option><option value="payment">Payment</option><option value="fee">Fee</option><option value="refund">Refund</option><option value="other">Other</option></select></label>
									<label>Type<select aria-label="Transaction type" bind:value={rowDraft.type}><option value="expense">Expense</option><option value="income">Income / credit</option></select></label>
									<label>Account<select aria-label="Account" bind:value={rowDraft.accountId}>{#each accounts as account}<option value={account.id}>{account.name}</option>{/each}</select></label>
									<label>Category<select aria-label="Category" bind:value={rowDraft.categoryId}><option value="">Choose category</option>{#each visibleCategories(rowDraft) as category}<option value={category.id}>{category.name}</option>{/each}</select></label>
								</div>
								<StatementMatchReview row={rowDraft} {entries} {accounts} {categories} removing={removingRowId === rowDraft.id} onChooseMatch={chooseMatch} onChooseNew={() => chooseDecision('new')} onRemove={removeRow} />
								<div class="editor-footer"><div>{#if rowDraft.warningCodes.length}<span class="warning-text">{rowDraft.warningCodes.join(', ').replace(/_/g, ' ')}</span>{/if}</div><div class="editor-actions"><button type="button" on:click={cancelEdit}>Cancel</button><button class="inline-primary" type="button" on:click={() => void saveDraft()} disabled={savingRowId === rowDraft.id || rowDraft.reviewStatus === 'unreviewed' || (rowDraft.reviewStatus === 'matched' && !rowDraft.matchExpenseId)}>{savingRowId === rowDraft.id ? 'Saving…' : rowDraft.reviewStatus === 'unreviewed' ? 'Choose an action' : 'Save decision'}</button></div></div>
							</div>
						{/if}
					</article>
...
<style>
	...
	.transactions-toolbar{border-bottom:1px solid var(--line)}.toolbar-controls{align-items:center;display:flex;gap:.65rem}.search-control,.combine-search{align-items:center;background:var(--field);border:1px solid var(--line);border-radius:11px;color:var(--muted);display:flex;gap:.45rem;padding:0 .75rem}.search-control input,.combine-search input{background:transparent;border:0;color:var(--ink);min-height:2.65rem;outline:0;padding:.55rem 0;width:min(21rem,32vw)}.toolbar-controls select{width:auto}.combine-panel{align-items:center;background:color-mix(in srgb,var(--primary) 7%,var(--panel));border-bottom:1px solid var(--line);display:flex;flex-wrap:wrap;gap:.65rem;padding:1rem 1.2rem}.combine-summary{align-items:center;color:var(--primary);display:flex;gap:.55rem;margin-right:auto}.combine-summary>div{display:grid;gap:.15rem}.combine-summary small{color:var(--muted);font-size:.76rem}.combine-panel select{background:var(--field);border:1px solid var(--line);border-radius:10px;color:var(--ink);min-height:2.65rem;max-width:23rem;padding:.5rem .7rem}.combine-cancel,.uncombine-row{background:transparent;border:1px solid var(--line);border-radius:10px;color:var(--muted);font-weight:750;min-height:2.55rem;padding:.5rem .7rem}.transaction-head,.row-summary{align-items:center;display:grid;gap:.8rem;grid-template-columns:7.4rem minmax(13rem,1.4fr) minmax(7.5rem,.6fr) minmax(8rem,.7fr) 7.4rem 1.2rem}.transaction-head{background:color-mix(in srgb,var(--panel) 90%,var(--paper));color:var(--muted);font-size:.72rem;font-weight:850;letter-spacing:.06em;padding:.65rem 1.2rem;text-transform:uppercase}.transaction-list{display:block;gap:0}.transaction-row{align-items:stretch;background:transparent;border:0;border-radius:0;border-top:1px solid var(--line);box-shadow:none;display:block;gap:0;grid-template-columns:none;padding:0;position:relative;transition:background .15s ease}.transaction-row strong{color:inherit;font-size:inherit}.transaction-head+.transaction-list .transaction-row:first-child{border-top:0}.transaction-row.selected-for-combine{background:color-mix(in srgb,var(--primary) 9%,var(--panel))}.row-selector{align-items:center;cursor:pointer;display:flex;height:4.6rem;left:0;padding:0 .75rem;position:absolute;top:0;z-index:2}.row-selector input{height:1px;opacity:0;position:absolute;width:1px}.selection-check{background:var(--field);border:1px solid color-mix(in srgb,var(--muted) 70%,var(--line));border-radius:5px;display:block;height:1.1rem;position:relative;transition:.15s ease;width:1.1rem}.row-selector input:checked+.selection-check{background:var(--primary);border-color:var(--primary);box-shadow:0 0 0 3px color-mix(in srgb,var(--primary) 16%,transparent)}.row-selector input:checked+.selection-check::after{border-bottom:2px solid #0a1630;border-right:2px solid #0a1630;content:'';height:.48rem;left:.35rem;position:absolute;top:.16rem;transform:rotate(45deg);width:.25rem}.row-selector input:focus-visible+.selection-check{outline:2px solid var(--primary);outline-offset:3px}.row-selector input:disabled+.selection-check{cursor:not-allowed;opacity:.38}.row-summary{background:transparent;color:var(--ink);min-height:4.6rem;padding:.75rem 3.1rem;text-align:left;width:100%!important}.row-summary:hover{background:var(--panel-hover)}.transaction-row.editing{background:color-mix(in srgb,var(--primary) 4%,var(--panel))}.transaction-row.row-warning{box-shadow:inset 3px 0 #edbb60}.merchant-cell,.amount-cell{display:grid;gap:.2rem;min-width:0}.merchant-cell strong,.merchant-cell small{overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.merchant-cell small,.amount-cell small{font-size:.74rem}.amount-cell strong.credit{color:#3dccaa}.category-cell{color:var(--muted);overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.row-chevron{color:var(--muted);transition:.15s}.transaction-row.editing .row-chevron{transform:rotate(90deg)}.row-status{flex-wrap:wrap}.combined-badge{background:#6794ff2b;border-radius:999px;color:var(--primary);font-size:.67rem;padding:.2rem .35rem}.uncombine-row{align-items:center;color:var(--primary);display:flex;gap:.35rem;margin:.2rem 3.1rem .8rem auto}.sr-only{height:1px;margin:-1px;overflow:hidden;padding:0;position:absolute;width:1px;clip:rect(0,0,0,0);white-space:nowrap}
	...
	@media(max-width:767px){.statement-ingestion{gap:1rem}.page-heading,.detail-heading,.transactions-toolbar,.editor-footer{align-items:stretch;flex-direction:column}.page-heading .primary-action{align-self:stretch}.drop-zone{grid-template-columns:auto 1fr;min-height:8rem;padding:1.15rem}.drop-zone .secondary-action{grid-column:1/-1;width:100%}.drop-icon{height:3.4rem;width:3.4rem}.drop-copy strong{font-size:1rem}.upload-options{align-items:stretch;flex-direction:column;margin-top:-.35rem}.upload-options label{align-items:stretch;display:grid;flex-basis:auto}.privacy-note{align-items:flex-start}.inline-primary{width:100%!important}.recent-heading{padding:1rem}.statement-item,.statement-item.local-item{gap:.8rem;grid-template-columns:auto 1fr auto;min-height:auto;padding:1rem}.statement-name{grid-column:2}.statement-summary{grid-column:2}.statement-period{display:none}.status-pill{grid-column:2;justify-self:start}.chevron{grid-column:3;grid-row:1/4}.local-actions{grid-column:3;grid-row:1/4;flex-direction:column}.detail-title{align-items:flex-start}.detail-title h2{white-space:normal;word-break:break-word}.detail-actions{display:grid}.detail-actions button{width:100%}.summary-grid{grid-template-columns:repeat(2,minmax(0,1fr))}.summary-grid article{padding:.85rem}.checks-card{border-radius:14px}.checks-list{display:grid;width:100%}.toolbar-controls{align-items:stretch;flex-direction:column}.search-control input,.combine-search input{width:100%}.toolbar-controls select,.combine-panel select{width:100%;max-width:none}.combine-panel{align-items:stretch;display:grid}.combine-summary{margin-right:0}.transaction-head{display:none}.row-selector{height:2.8rem;padding:0 .7rem}.row-summary{gap:.55rem;grid-template-columns:1fr auto;padding:.9rem 1rem .9rem 2.8rem}.row-summary>span::before{color:var(--muted);content:attr(data-label);display:block;font-size:.65rem;font-weight:800;letter-spacing:.06em;margin-bottom:.18rem;text-transform:uppercase}.date-cell{grid-column:1;grid-row:1}.amount-cell{grid-column:2;grid-row:1;text-align:right}.merchant-cell{grid-column:1/-1;grid-row:2}.category-cell{grid-column:1;grid-row:3}.row-status{grid-column:2;grid-row:3;justify-self:end}.row-chevron{display:none}.uncombine-row{position:static;margin:.15rem 1rem .8rem auto}.row-editor{padding:1rem}.editor-grid{grid-template-columns:1fr}.editor-grid .merchant-field,.editor-grid .match-field{grid-column:auto}.editor-actions{display:grid;grid-template-columns:1fr 1fr}.editor-actions button{width:100%}}
	.bank-mark.danger{background:#ef5e5326;color:#ef8f87}
	.combine-panel{display:grid;gap:1rem;margin:0;min-width:0;padding:1.3rem 1.4rem;background:var(--panel)}
	.combine-summary{margin:0;min-width:0}.combine-summary>div{flex:1;min-width:0}.combine-summary small{overflow-wrap:anywhere}.combine-total{color:var(--ink);font-size:1.35rem;font-variant-numeric:tabular-nums;white-space:nowrap}
	.combine-mode{display:flex;gap:.35rem;padding:.35rem;border:1px solid var(--line);border-radius:12px;background:var(--field)}.combine-mode button{background:transparent;border:0;border-radius:8px;color:var(--muted);flex:1;min-height:2.8rem;padding:.65rem 1rem;font-weight:750}.combine-mode button[aria-pressed=true]{background:color-mix(in srgb,var(--primary) 16%,var(--panel));color:var(--primary)}
	.combine-fields{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:.85rem}.combine-fields label{display:grid;gap:.4rem;font-size:.8rem;color:var(--muted);min-width:0}.combine-fields .wide-field{grid-column:1/-1}.combine-fields input,.combine-fields select{background:var(--field);border:1px solid var(--line);border-radius:10px;color:var(--ink);min-height:2.8rem;padding:.6rem .8rem;width:100%;max-width:none;min-width:0;font-size:1rem}.combine-hint{font-size:.82rem;overflow-wrap:anywhere}.combine-actions{display:flex;gap:.65rem;justify-content:flex-end}.combine-cancel{align-items:center;display:inline-flex;gap:.4rem;justify-content:center}
	.combine-panel .combine-search input{width:100%;min-width:0;font-size:1rem}.combine-panel .combine-search{min-width:0}.combine-candidates{display:grid;gap:.6rem;max-height:20rem;overflow-y:auto}.combine-candidate{display:flex;align-items:center;gap:.75rem;border:1px solid var(--line);border-radius:12px;padding:.85rem;cursor:pointer;min-width:0}.combine-candidate:has(input:checked){background:color-mix(in srgb,var(--primary) 14%,var(--panel));border-color:var(--primary)}.combine-candidate input{accent-color:var(--primary);width:1.1rem;height:1.1rem;min-width:1.1rem;padding:0}.combine-candidate span{display:grid;gap:.2rem;flex:1;min-width:0;overflow-wrap:anywhere}.combine-candidate>strong{font-variant-numeric:tabular-nums;white-space:nowrap}.combine-candidate small{font-size:.78rem}
	.saved-combination{display:flex;align-items:center;gap:.8rem;padding:1rem 1.3rem;border-bottom:1px solid var(--line);background:color-mix(in srgb,var(--primary) 7%,var(--panel))}.saved-combination>div{display:grid;gap:.2rem;flex:1;min-width:0;overflow-wrap:anywhere}.saved-combination small{font-size:.8rem}.saved-combination :global(svg){flex-shrink:0;color:var(--primary)}
	@media(max-width:767px){.combine-fields{grid-template-columns:1fr}.combine-summary{flex-wrap:wrap}.combine-total{width:100%;margin-top:.3rem}.combine-mode{flex-direction:column}.combine-actions{flex-wrap:wrap}.combine-actions button{flex:1}.saved-combination{flex-wrap:wrap}.saved-combination>div{flex-basis:75%}.combine-candidate{flex-wrap:wrap}.combine-candidate>strong{margin-left:1.85rem}.combine-panel{padding:1rem}}
</style>
